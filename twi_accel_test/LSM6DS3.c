...
void write_LSM6DS3(uint8_t address, uint8_t value){
    ...
    twi_start_wr(LSM6DS3_ADDRESS, wr_buf, 2);
...
void read_LSM6DS3(uint8_t address, uint8_t *rd_buf){
    ...
    twi_start_wr(LSM6DS3_ADDRESS, addr_buf, 1);

    // Recieve data from register address and store in rd_buf
    twi_start_rd(LSM6DS3_ADDRESS, rd_buf, 1);
...
void init_accel(){
    ...
    // Set accelerometer ODR to 208Hz and acceleration range to +/- 8G's
    set_bits(CTRL1_XL, ODR_XL0 | ODR_XL2 | FS_XL0 | FS_XL1);
...
void init_gyro(){
    ...
    // Set gyroscope ODR to 208Hz angular rate to to 1000 degrees/second
    set_bits(CTRL2_G, ODR_G0 | ODR_G2 | FS_G1);
